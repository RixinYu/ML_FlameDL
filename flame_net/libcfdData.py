...
class libcfdData:
    ...
    def demo( case = 1 ):
        ...
        if case ==1 :
            ...
            libcfdData.get_yBminmax_from_PREdata(cfd_data_dir=cfd_data_dir)
        elif case ==3:
            libcfdData.animate_cfd_file('L512_rho8')
    ...
    @staticmethod
    def load_2DPREdata(list_picklefilename =None, cfd_data_dir= None, Nx_target=None,yB_estimate=None,AspectRatio_set=1  ): #Nx_target, default keep the number of mesh-points the same as the one stored inside PRE file
        if list_picklefilename==None:
            list_picklefilename = [ 'L512_rho5.pkl',  'L512_rho8.pkl',   'L512_rho10.pkl'  ]
        print(list_picklefilename)
        ...
        for picklefilename in list_picklefilename:
            ...
            loaded_list = pickle.load(open_file)
            open_file.close()
            p = np.array( [loaded_list['rhoR'], loaded_list['L'] ] )
            list_p.append(p)
            del loaded_list
            ##############################################
    ...
    def get_levelsetG2D(picklefilename, Nx_target, yB_estimate=None, ipick_single=None, AspectRatio_set = 1):
        #picklefilename = 'd:\\Work\\00_MLproj\\Data_PRE_LaminarFlame\\' + 'L512_rho8.pkl'
        ...
        for  (y_x, ylevelG ) in  zip(xy_curv,   ylevelG2D_all) :

            for i_skip in range(Nx_target):
                i = i_skip * nSkip # i is the index on the underlying fine mesh

                idx = np.where( y_x[:, 1] == i)[0]  # note: this is the x-coordinate
                #------------------
                y_tmp = y_x[idx, 0]
                #------------------
