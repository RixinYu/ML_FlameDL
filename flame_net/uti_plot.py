...
from libSiva_update import libSiva, libData
...
class libPlot:
    ...
    def animlevel2D_realtime_model(model, Nx,Ny, yB, nStep=1,nSkipStep_plot=1, nReset=50, nrows=2, ncols=2 ,
                                                 method_levelset='levelset_fast', bTrainedByTanh=True, yRef0=0 ):
        ...
        def update_animation_realtimecaculate2D(num, model,yRef0 ): #, SivaEq):
            ...
            if num % nReset == 0:
                ...
                for idx in range(G2D_model.shape[0]):
                    ...
                    G2D_model[idx, :, :, 0] =  torch.tensor(  func_on_ylevel( libData.dsol_single_to_Levelsetsol(x, d0, Ny, yB, method_levelset)   ) - yRef0,  dtype=torch.float)
    ...
    @staticmethod
    def animlevel2D_realtimeCmp_model_analytical( SivaEq, model,nu_check=0.07, nStep=1, nReset=500,x=None, nrows=2,ncols=2, ylim=(-0.7*np.pi,1.3*np.pi) ,  method_levelset='levelset_fast',bTrainedByTanh=True ):
        ...
        def update_animation_realtimecaculate2D(num,model,SivaEq) : 
            ...
            if num % nReset == 0:
                ...
                for idx in range(d_an.shape[0]):
                    G2D_model[idx,:,:,0] = torch.tensor( func_on_ylevel( libData.dsol_single_to_Levelsetsol(x,d_an[idx,:,0],Ny=Ny,method_levelset=method_levelset) ),dtype = torch.float )
                    #G2D_model[idx,:,:,0] = torch.tensor( ( libData.dsol_single_to_Levelsetsol(x,d_an[idx,:,0],Ny=Ny,method_levelset=method_levelset) ),dtype = torch.float )
                #d_model = torch.repeat_interleave( torch.tensor(d_an,dtype = torch.float), data_channel, dim= -1) 
    ...
    @staticmethod
    def anim_realtimeCmp_model_analytical( SivaEq, model,nu_check=0.07, nReset=500,x=None, nrows=2,ncols=2, ylim=(-1,3) ):
        ...
        def update_animation_realtimecaculate(num,model,SivaEq) : 
            ...
            return 
        ani = animation.FuncAnimation(fig, update_animation_realtimecaculate, nReset, fargs=(model,SivaEq), interval=1,blit=False)
        return ani    
    ...
    @staticmethod
    def Take_4RandInit_plot_Asequence(SivaEq, model, nu=0.07, numTotalTimeStep=1000):
        T_in = model.T_in
        data_channel = model.data_channel
        ...
        fig, axs = plt.subplots(2,2, sharex=True, sharey=True, figsize= (9,5))
